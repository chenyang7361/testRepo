...
/**
 * 通过第三方sdk（如微博，QQ登录）拿到的accessToken来进行后台登录
...
class AccessTokenAuthenticator(
    ...
    private fun flatMapAccountInfo(authResult: AuthResult): Observable<AccountInfo> {
        return duoShouRepository.accessTokenLogin(channel, authResult.uid, "kuai",
                authResult.accessToken, BuildConfig.ACCOUNT_KIND)
                .map { t: JSONObject? -> mapAccountInfo(t) }
