 ...
 * 首页视频分类集合
 */
class HeadlineTabViewModel(val context: Context) : BaseViewModel(context), KodeinInjected {
    ...
    companion object {
        val DEFAULT_TAB_DATA = "headline_tabs.json"
        val HEADLINE_TAB_ATYPE = 2
    }
        ...
        if (!subscription.hasSubscriptions()) {
            Observable.create<Boolean> { subscribe ->
                ...
                subscribe.onCompleted()
                    ...
                    .flatMap {
                        mainRepository.getTabs(HEADLINE_TAB_ATYPE)
                    }
                    ...
                    .subscribe({ tabList ->
                        if (tabList?.data != null) {
                            this@HeadlineTabViewModel.tabs = tabList.data
